...
approver_id_password = os.getenv("test_approver_id_password")
approver_id_2_password = os.getenv("test_approver_id_2_password")
bill_recommender_2_password = os.getenv("test_bill_recommender_2_password")
bill_recommender_3_password = os.getenv("test_bill_recommender_3_password")
...
proc_admin_password = os.getenv("test_proc_admin_password")
sso_login_receiver_pin = os.getenv("test_sso_login_receiver_pin")

delegated_approver = os.getenv("test_delegated_approver")
delegated_approver_password = os.getenv("test_delegated_approver_password")
...
# delegated_order_reference_number = ''
actual_approver_pending_approval_orders = ''
delegated_approver_pending_approval_orders = ''
framework_order_no = ''
vendor_login_id = ''
...
def test_1_create_requisition_with_whitelisted_agreement_item(page, logger):
    ...
    create_requisition_page.setting_active_framework_list(agreement_info="BPD/2026/FA-5")
    create_requisition_page.agreement_item_selector.nth(0).click()
    global requisition_item_quantity
    requisition_item_quantity = str(create_requisition_page.finalize_item_quantity(item_quantity="100"))
    # logger.step("Requisition item quantity: " + requisition_item_quantity)
    create_requisition_page.setting_requisition_for_details("1104010301-02",
                                                            "Item remarks abc123@")
    # create_requisition_page.setting_requisition_for_details("[1104010301-02] Advance to Staff against Expenses",
    #                                                         "Item remarks abc123@")
    ...
    logger.step(f"Requisition number: {req_num}")
...
def test_2_identify_first_approver_for_created_requisition(page, logger):
    ...
    logger.step(f"Requisition first layer approver ID: {approver_id}")
...
def test_3_login_as_first_approver_and_approve_requisition(page, logger):
    ...
    #     pass_word=approver_id_password,
    #     timeout=60000
    # )
    reset_page = ResetHubPage(page)
...
def test_4_identify_second_approver_for_created_requisition(page, logger):
    ...
    logger.step(f"Requisition second or final layer approver ID: {approver_id_2}")
    # logger.step("Requisition second/final layer ID: ", approver_id_2)
...
def test_5_login_as_second_approver_and_approve_requisition(page, logger):
    ...
    #     pass_word=approver_id_2_password,
    ...
    reset_page.open_generated_link(link)
    assert isinstance(link, str) and link.startswith("http")
    logger.step(f" 📥 Logging in as final layer approver: {approver_id_2}")
...
def test_6_verify_requisition_is_approved(page, new_tab, logger):
    ...
    # logger.step("Requisition status: ", req_status)
    ...
    logger.step(f"Requisition Vendor Name: {order_vendor}")
    # logger.step("Requisition Vendor Name: ", order_vendor)
...
def test_7_initiate_marketplace_order(page, new_tab, logger):
    ...
    logger.step(f"Logging in as an order initiator: {proj_user}")
    # logger.step("Logging in as an order initiator: " + order_initiator)
    ...
    updated_cart_quantity = cart_page.update_shopping_cart_value_1(qty_update="10")
    # logger.step(f"Updated cart quantity: {str(updated_cart_quantity)}")
    # logger.step("Updated cart quantity: " + str(updated_cart_quantity))
    ...
    logger.step(f"Marketplace order reference number: {order_reference_number}")
    # logger.step("Marketplace order reference number: ", order_reference_number)
    checkout_page.wait_for_timeout(2000)
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 8: Verify Marketplace Order in Approver's Pending Approval List

       Objective:
        Verify that the designated order approver can successfully log in to the
        Marketplace, navigate to the Pending Approval Orders list, verify the
        pending approval order count, and search for the target Marketplace order
        using its order reference number.

       Steps:
        1. Navigate to the Marketplace QA environment.
        2. Log in using the designated order approver's credentials.
        3. Navigate to the Orders section.
        4. Open the Pending Approval Orders list.
        5. Retrieve and log the approver's pending approval order count.
        6. Search for the target order using the generated order reference number.
        7. Verify that the search operation is performed successfully.
        8. Log out from the Marketplace. """)
def test_8_verify_marketplace_order_in_approvers_pending_approval_list(page, logger):
    """
       Test Case 8: Verify Marketplace Order in Approver's Pending Approval List
       ...
       Steps:
        ...
        8. Log out from the Marketplace.
       """
    print("Test 8: Verify Marketplace order in approver's pending approval list...")
    login_page = LoginPage(page)
    login_page.perform_login_for_sso_login(
        user_name=approver_id_2,
        pass_word=marketplace_password
    )
    logger.step(f" 📥 Logging in as order actual approver: {approver_id_2}")
    ...
    home_page.get_full_page_screenshot('full_page_screenshot_33')

    pending_approval_orders = PendingApprovalOrders(page)
    global actual_approver_pending_approval_orders
    pending_approval_orders.get_pending_approval_order_count(
        pending_approval_order_count=actual_approver_pending_approval_orders)
    logger.step(f"Actual approver pending approval orders count: " + str(
        pending_approval_orders.get_pending_approval_order_count(
            pending_approval_order_count=actual_approver_pending_approval_orders)))
    # logger.step("Actual approver pending approval orders count: " + str(
    #     pending_approval_orders.get_pending_approval_order_count(
    #         pending_approval_order_count=actual_approver_pending_approval_orders)))
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""
    Test Case 9: Verify My Delegated Orders Information Before Delegation

    Objective:
        Verify that the designated delegated approver can log in to the Marketplace,
        navigate to the Orders section, access the My Delegated Orders option if
        available, and retrieve the current delegated order count before delegation
        is applied.

    Steps:
        1. Navigate to the Marketplace QA environment.
        2. Log in using the designated delegated approver's credentials.
        3. Navigate to the Orders section.
        4. Check whether the My Delegated Orders option is available.
        5. Open My Delegated Orders if the option is available.
        6. Retrieve and log the current delegated order count before delegation.
        7. Log out from the Marketplace.
    """
)
def test_9_verify_my_delegated_orders_before_delegation(page, logger):
    """
    Test Case 9: Verify My Delegated Orders Information Before Delegation
    ...
    print("Test 9: Verify My Delegated Orders information before delegation...")
    login_page = LoginPage(page)
    login_page.perform_login_for_sso_login(
        user_name=delegated_approver,
        pass_word=marketplace_password
    )
    logger.step(f" 📥 Logging in as order delegated approver: {delegated_approver}")

    home_page = HomePage(page)
    home_page.get_full_page_screenshot('full_page_screenshot_35')
    home_page.goto_order_list()
    home_page.get_full_page_screenshot('full_page_screenshot_36')

    my_account_orders_list = OrdersPublicStore(page)
    delegated_order_count = (
        my_account_orders_list
        .go_to_my_delegated_orders_if_available()
    )

    logger.step(
        f"Delegated orders count: {delegated_order_count}"
    )
    dm_logout = MainNavigationMenu(page)
    dm_logout.perform_logout()
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""
    Test Case 10: Create DM Order Approval Delegation with TOA Category

    Objective:
        Verify that a Delegation of Authority can be successfully created for
        Digital Marketplace Order Approval by selecting the required TOA category,
        delegated approver, module, delegation period, and remarks, and verify the
        created delegation from the Delegation of Authority List.

    Steps:
        1. Log in to the Procurement system using valid credentials.
        2. Navigate to the Delegation of Authority page.
        3. Select the designated delegated approver.
        4. Enable the TOA Category Required option.
        5. Select the Procurement module.
        6. Search for and add the DM Order Approval TOA category.
        7. Select the delegation start date.
        8. Select the delegation end date.
        9. Enter delegation remarks.
        10. Create the delegation and verify the confirmation.
        11. Navigate to the Delegation of Authority List.
        12. Search for the delegated approver using the employee PIN.
        13. Log the delegated approver information.
        14. Exit and log out from the Procurement system.
    """
)
def test_10_create_dm_order_approval_delegation_with_toa_category(page, logger):
    """
    Test Case 10: Create DM Order Approval Delegation with TOA Category
    ...
    print("Test 10: Create DM Order Approval Delegation with TOA Category...")
    ...
    proc_dashboard_page.get_full_page_screenshot('full_page_screenshot_37')

    proc_delegation_of_authority_page = DelegationOfAuthority(page)
    proc_delegation_of_authority_page.select_delegated_employee(employee_search_text=delegated_approver)
    proc_delegation_of_authority_page.select_toa_category_required()
    proc_delegation_of_authority_page.select_module(module_name=module_selection)
    proc_delegation_of_authority_page.search_and_add_toa_category(category_name=dm_order_approval_category)
    proc_delegation_of_authority_page.select_start_date(proc_delegation_of_authority_page.select_date())
    proc_delegation_of_authority_page.select_end_date(proc_delegation_of_authority_page.select_date())
    proc_delegation_of_authority_page.enter_remarks(remarks=delegation_remarks)
    proc_delegation_of_authority_page.get_full_page_screenshot('full_page_screenshot_38')
    proc_delegation_of_authority_page.click_create_button_and_delegation_confirmation()
    proc_delegation_of_authority_page.get_full_page_screenshot('full_page_screenshot_39')

    proc_delegation_of_authority_list_page = DelegationOfAuthorityListPage(page)
    proc_delegation_of_authority_list_page.go_to_delegation_of_authority_list()
    proc_delegation_of_authority_list_page.wait_for_timeout(2000)
    proc_delegation_of_authority_list_page.get_full_page_screenshot('full_page_screenshot_40')
    proc_delegation_of_authority_list_page.search_by_delegated_approver_by_PIN(PIN=delegated_approver)
    logger.step(f"Delegated approver: {delegated_approver}")
    # logger.step("Delegated approver:" + delegated_approver)
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""
    Test Case 11: Approve Marketplace Order by Delegated Approver After Delegation Creation

    Objective:
        Verify that after a valid Delegation of Authority is created for DM Order
        Approval, the delegated approver can access the My Delegated Orders section,
        locate the delegated Marketplace order, view its details, successfully
        approve the order, and verify the updated order status.

    Steps:
        1. Navigate to the Marketplace environment.
        2. Log in using the delegated approver's credentials.
        3. Navigate to the Orders section.
        4. Access the My Delegated Orders section.
        5. Retrieve and log the delegated order count after delegation creation.
        6. Search for the delegated order using the order reference number.
        7. View the delegated order information.
        8. Navigate to the delegated order details page.
        9. Approve the delegated Marketplace order.
        10. Retrieve and log the order status after approval.
        11. Capture the required screenshots for test evidence.
        12. Log out from the Marketplace.
    """
)
def test_11_approve_marketplace_order_by_delegated_approver_after_delegation(page, logger):
    """
    Test Case 11: Approve Marketplace Order by Delegated Approver After Delegation Creation
    ...
    print(
        "Test 11: Approve Marketplace order by delegated approver after delegation creation..."
    )
    login_page = LoginPage(page)
    login_page.navigate_to_url(marketplace_url_qa)
    login_page.perform_login_for_sso_login(
    ...
    logger.step(
        f"Logging in as delegated approver for approving delegated order: {delegated_approver}")

    home_page = HomePage(page)
    home_page.get_full_page_screenshot('full_page_screenshot_41')
    home_page.goto_order_list()
    home_page.get_full_page_screenshot('full_page_screenshot_42')

    my_account_orders_list = OrdersPublicStore(page)
    my_delegated_order_count = (
        my_account_orders_list
        .go_to_my_delegated_orders_if_available()
    )

    logger.step(
        f"Delegated orders count: {my_delegated_order_count}"
    )
    my_account_orders_list.get_full_page_screenshot('full_page_screenshot_43')

    my_account_my_delegated_orders = MyDelegatedOrders(page)
    my_account_my_delegated_orders.search_delegated_order(
        # delegated_order_reference_number="2026/TRN-5455"
        # reference_number=order_reference_number
        delegated_order_reference_number=order_reference_number
    )
    my_account_my_delegated_orders.click_delegated_order_search_button()
    my_account_my_delegated_orders.view_delegated_order_info_toggle()
    my_account_my_delegated_orders.get_full_page_screenshot('full_page_screenshot_44')
    my_account_my_delegated_orders.goto_my_delegated_order_details()
    my_account_my_delegated_orders.get_full_page_screenshot('full_page_screenshot_45')
    # my_account_my_delegated_orders.approve_my_delegated_order()

    my_delegated_order_status = (
        my_account_my_delegated_orders.approve_my_delegated_order()
    )

    logger.step(
        f"Delegated orders status after order approval: {my_delegated_order_status}"
    )
    my_account_my_delegated_orders.wait_for_timeout(2000)
    my_account_my_delegated_orders.get_full_page_screenshot('full_page_screenshot_46')
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 12: Retrieve vendor credentials for a specific marketplace order.
...
def test_12_retrieve_vendor_credentials_for_marketplace_order(page, logger):
    """
    Test Case 12: Retrieve vendor credentials for a specific marketplace order.
    ...
    Steps:
        ...
        10. Log out from the administration portal.
    """
    print("Test 12: Find vendor credential for order...")
    login_page = LoginPage(page)
    login_page.perform_login_for_sso_login(
        user_name=dm_admin,
        pass_word=marketplace_password
    )
    logger.step(f"Logging in as the system admin: {dm_admin}")
    home_page = HomePage(page)
    home_page.goto_all_orders_for_admin()
    home_page.get_full_page_screenshot('full_page_screenshot_47')

    all_orders = AllOrderForAdminPage(page)
    all_orders.admin_order_search(
        search_number=order_reference_number
    )
    all_orders.get_full_page_screenshot('full_page_screenshot_48')
    all_orders.admin_goes_to_order_details()
    all_orders.get_full_page_screenshot('full_page_screenshot_49')
    all_orders.goto_admin_dashboard()
    all_orders.get_full_page_screenshot('full_page_screenshot_50')
    ...
    logger.step(f"Order vendor login ID: {vendor_login_id}")
    # logger.step("Order vendor login ID:", vendor_login_id)
    customers_page.get_full_page_screenshot('full_page_screenshot_51')
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 13: Marketplace vendor acknowledgement process.
...
def test_13_vendor_acknowledges_marketplace_order(page, logger):
    """
    Test Case 13: Marketplace vendor acknowledgement process.
    ...
    Steps:
        ...
        10. Log out from the marketplace administration portal.
    """
    print("Test 13: Vendor acknowledgement process...")
    login_page = LoginPage(page)
    login_page.perform_vendor_login(
        user_name=vendor_login_id,
        pass_word=stg_vendor_pass
    )
    logger.step(f"Logging in as vendor: {vendor_login_id}")
    vendor_dashboard = VendorDashboard(page)
    vendor_dashboard.get_full_page_screenshot('full_page_screenshot_52')
    vendor_dashboard.print_card_title()
    vendor_dashboard.print_table_data()
    vendor_dashboard.get_full_page_screenshot('full_page_screenshot_53')
    vendor_dashboard.click_action_for_order(order_reference=order_reference_number)
    vendor_dashboard.get_full_page_screenshot('full_page_screenshot_54')
    ...
    logger.step(f"Generate framework order number: {framework_order_no}")
    order_details_administration.wait_for_timeout(5000)
    order_details_administration.get_full_page_screenshot('full_page_screenshot_55')
    order_details_administration.click_back_to_order_list()
    order_details_administration.get_full_page_screenshot('full_page_screenshot_56')
    ...
    orders_list_management.order_search_button.click()
    orders_list_management.search_order(order_no=framework_order_no)
    orders_list_management.get_full_page_screenshot('full_page_screenshot_57')
    orders_list_management.wait_for_timeout(5000)
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 14: View marketplace framework order details in the procurement system.
...
def test_14_login_to_procurement_and_view_marketplace_work_order_details(page, new_tab, logger):
    """
    Test Case 14: View marketplace framework order details in the procurement system.
    ...
    Steps:
        ...
        9. Log out from the procurement system and capture a final screenshot.
    """
    print("Test 14: Marketplace framework order details view in procurement system...")
    # proc_login_page = ProcurementLoginPage(page)
    # proc_login_page.perform_login(
    #     given_url=proj_url,
    #     user_name=proc_admin,
    #     pass_word=proc_admin_password,
    #     timeout=60000
    # )
    reset_page = ResetHubPage(page)
    ...
    reset_page.open_generated_link(link)
    assert isinstance(link, str) and link.startswith("http")
    logger.step(f" 📥 Logging in as procurement admin: {proc_admin}")

    proc_dashboard_page = DashboardPage(page)
    proc_dashboard_page.goto_procurement()
    proc_dashboard_page.get_full_page_screenshot('full_page_screenshot_58')

    proc_home_page = ProcurementHomePage(page)
    proc_home_page.navigate_to_framework_order_list()
    proc_home_page.get_full_page_screenshot('full_page_screenshot_59')
    ...
    framework_order_list_page.get_full_page_screenshot('full_page_screenshot_60')
    ...
    framework_order_list_page.get_full_page_screenshot('full_page_screenshot_61')
    new_page.close()

    m_page = MainNavigationBar(page)
    m_page.exit()
    m_page.logout()
    m_page.get_full_page_screenshot('full_page_screenshot_62')


@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 15: Marketplace item receipt process by the designated receiver.
...
def test_15_receiver_receives_marketplace_item(page, logger):
    """
    Test Case 15: Marketplace item receipt process by the designated receiver.
    ...
    Steps:
        ...
        12. Log out from the administration portal.
    """
    print("Test 15: Item received by receiver...")
    login_page = LoginPage(page)
    login_page.navigate_to_url(marketplace_url_qa)
    login_page.perform_login_for_sso_login(
    ...
    logger.step("Logging in as item receiver: " + receiving_pin_1)
    home_page = HomePage(page)
    home_page.goto_administration()
    home_page.get_full_page_screenshot('full_page_screenshot_62')
    home_page.wait_for_timeout(2000)
    ...
    receivable_order_list.search_button.click()
    receivable_order_list.search_receivable_order(receivable_order_number=framework_order_no)
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_64')
    ...
    logger.step(f"Print generated challan number for receiver: {challan_num_for_receiver}")

    receivable_order_list.all_item_select.click()

    current_dir = os.getcwd()
    document_location = os.path.join(current_dir, "utils", "upload_file.pdf")
    assert receivable_order_list.receiving_upload_attachment(document_location), "File upload failed"

    receivable_order_list.wait_for_timeout(5000)

    receivable_order_list.input_received_remarks(receiving_remarks="Received remarks test 123 !@#")
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_65')
    receivable_order_list.open_item_receive_popup()
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_66')
    receivable_order_list.confirm_receivable_order()
    receivable_order_list.wait_for_timeout(5000)
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_67')
    ...
    item_received_list.search_received_order(received_order_number=framework_order_no)
    item_received_list.searched_received_order(
        challan_no=challan_num_for_receiver
    )

    item_received_list.search_button_for_received_item.click()
    item_received_list.get_full_page_screenshot('full_page_screenshot_68')
    item_received_list.order_view_button.click()
    item_received_list.get_full_page_screenshot('full_page_screenshot_69')
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 16: Marketplace item receipt by order initiator acting as receiver.
...
def test_16_order_initiator_receives_marketplace_item(page, logger):
    """
    Test Case 16: Marketplace item receipt by order initiator acting as receiver.
    ...
    Steps:
        ...
        13. Log out from the administration portal.
    """
    print("Test 16: Item received by receiver as order initiator...")
    login_page = LoginPage(page)
    login_page.perform_login_for_sso_login(
        user_name=proj_user,
        pass_word=marketplace_password
    )
    logger.step(f"Logging in as an order initiator for item receive: {proj_user}")

    home_page = HomePage(page)
    home_page.goto_administration()
    home_page.get_full_page_screenshot('full_page_screenshot_70')
    home_page.wait_for_timeout(2000)
    ...
    receivable_order_list.search_button.click()
    receivable_order_list.search_receivable_order(receivable_order_number=framework_order_no)
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_71')
    receivable_order_list.receivable_order_view()

    global challan_num_for_order_initiator
    receivable_order_list.challan_no_input(fill_challan_no=challan_num_for_order_initiator)
    print("Print generated challan number for order initiator: ", challan_num_for_order_initiator
          )
    logger.step(f"Print generated challan number for order initiator: {challan_num_for_order_initiator}")
    receivable_order_list.all_item_select.click()
    receivable_order_list.wait_for_timeout(5000)
    receivable_order_list.input_quantity_to_receive(received_quantity="1")

    current_dir = os.getcwd()
    document_location = os.path.join(current_dir, "utils", "image_png.png")
    assert receivable_order_list.receiving_upload_attachment(document_location), "File upload failed"

    receivable_order_list.input_received_remarks(
        receiving_remarks="Partially received item 1")
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_72')
    receivable_order_list.open_item_receive_popup()
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_73')
    receivable_order_list.confirm_receivable_order()
    receivable_order_list.wait_for_timeout(5000)
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_74')
    ...
    item_received_list.search_button_for_received_item.click()
    item_received_list.get_full_page_screenshot('full_page_screenshot_75')
    item_received_list.order_view_button.click()
    item_received_list.wait_for_timeout(5000)
    item_received_list.get_full_page_screenshot('full_page_screenshot_76')
    ...
    receivable_order_list.search_button.click()
    receivable_order_list.search_receivable_order(receivable_order_number=framework_order_no)
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_77')
    receivable_order_list.receivable_order_view()

    global challan_num_for_order_initiator_2
    receivable_order_list.challan_no_input(fill_challan_no=challan_num_for_order_initiator_2)
    print("Print generated challan number for order initiator: ", challan_num_for_order_initiator_2
          )
    logger.step(f"Print generated challan number for order initiator: {challan_num_for_order_initiator_2}")
    ...
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_78')
    receivable_order_list.open_item_receive_popup()
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_79')
    receivable_order_list.confirm_receivable_order()
    receivable_order_list.wait_for_timeout(5000)
    receivable_order_list.get_full_page_screenshot('full_page_screenshot_80')
    ...
    item_received_list.search_button_for_received_item.click()
    item_received_list.get_full_page_screenshot('full_page_screenshot_81')
    item_received_list.order_view_button.click()
    item_received_list.wait_for_timeout(5000)
    item_received_list.get_full_page_screenshot('full_page_screenshot_82')
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 17: View marketplace item receive details in the procurement system.
...
def test_17_login_to_procurement_and_view_marketplace_item_receipt_details(page, logger):
    """
    Test Case 17: View marketplace item receive details in the procurement system.
    ...
    Steps:
        ...
        7. Log out from the procurement system and capture a final screenshot.
     """
    print("Test 17: Marketplace item receive details view in procurement system...")
    # proc_login_page = ProcurementLoginPage(page)
    # proc_login_page.perform_login(
    #     given_url=proj_url,
    #     user_name=proc_admin,
    ...
    proc_dashboard_page = DashboardPage(page)
    proc_dashboard_page.goto_procurement()
    proc_dashboard_page.get_full_page_screenshot('full_page_screenshot_83')

    proc_home_page = ProcurementHomePage(page)
    proc_home_page.goto_item_receive_list()
    proc_home_page.get_full_page_screenshot('full_page_screenshot_84')
    ...
    proc_item_receive_list_page.get_full_page_screenshot('full_page_screenshot_85')
    proc_item_receive_list_page.item_receive_details_view()
    proc_item_receive_list_page.get_full_page_screenshot('full_page_screenshot_86')

    m_page = MainNavigationBar(page)
    m_page.exit()
    m_page.logout()
    m_page.get_full_page_screenshot('full_page_screenshot_87')


@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 18: Bill creation and submission for Marketplace item receive in procurement system.
...
def test_18_create_and_submit_bill_for_received_marketplace_item(page, logger):
    """
    Test Case 18: Bill creation and submission for Marketplace item receive in procurement system.
    ...
    Steps:
        ...
        11. Capture a screenshot of the bill details.
    """
    print("Test 18: Bill creation and submission flow for Marketplace item receive in procurement system...")
    # proc_login_page = ProcurementLoginPage(page)
    # proc_login_page.perform_login(
    #     given_url=proj_url,
    ...
    reset_page.open_generated_link(link)
    assert isinstance(link, str) and link.startswith("http")
    logger.step(f" 📥 Logging in as bill creator: {proj_user}")

    proc_dashboard_page = DashboardPage(page)
    proc_dashboard_page.goto_procurement()
    proc_dashboard_page.get_full_page_screenshot('full_page_screenshot_88')

    proc_home_page = ProcurementHomePage(page)
    proc_home_page.goto_bill_payable()
    proc_home_page.get_full_page_screenshot('full_page_screenshot_89')
    ...
    logger.step(f"Print Bill Number: {bill_num}")
    ...
    # create_vendor_bill.Bill_recommender1_selecting(recommender="00000761")
    create_vendor_bill.Bill_recommender2_selecting(recommender="00009026")
    create_vendor_bill.get_full_page_screenshot('full_page_screenshot_90')
    create_vendor_bill.submit_bill()
    create_vendor_bill.get_full_page_screenshot('full_page_screenshot_91')
    create_vendor_bill.confirm_submission()
    create_vendor_bill.wait_for_timeout(5000)
    create_vendor_bill.get_full_page_screenshot('full_page_screenshot_92')

    vendor_billing_list_page = VendorBillingList(page)
    vendor_billing_list_page.go_to_billing_list()
    vendor_billing_list_page.get_full_page_screenshot('full_page_screenshot_93')
    vendor_billing_list_page.search_bill(bill_num)
    global bill_recommender_1
    bill_recommender_1 = str(int(vendor_billing_list_page.find_approver_id(bill_num)))
    print(f"Bill Recommender 1: {bill_recommender_1}")
    logger.step(f"Bill Recommender 1: {bill_recommender_1}")
    vendor_billing_list_page.get_full_page_screenshot('full_page_screenshot_94')


# Hide if needed
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 19: Vendor bill approval by first recommender in the procurement system.
...
def test_19_first_recommender_approves_marketplace_bill(page, new_tab, logger):
    """
    Test Case 19: Vendor bill approval by first recommender in the procurement system.
    ...
    Steps:
        ...
        8. Exit and log out from the procurement system, capturing a final screenshot.
    """
    print("Test 19: Vendor bill recommender1 approval...")
    vendor_billing_list_page = VendorBillingList(page)
    new_page = new_tab(lambda p: vendor_billing_list_page.click_on_bill_num(bill_num))

    bill_details_information_page = BillDetailsInformation(new_page)
    current_dir = os.getcwd()
    document_location = os.path.join(current_dir, 'utils', 'upload_file.pdf')
    bill_details_information_page.upload_document(document_location)
    bill_details_information_page.select_bill_type("Regular")
    bill_details_information_page.get_full_page_screenshot('full_page_screenshot_95')
    bill_details_information_page.approve_bill()
    bill_details_information_page.get_full_page_screenshot('full_page_screenshot_96')
    new_page.close()

    vendor_billing_list_page = VendorBillingList(page)
    vendor_billing_list_page.search_bill(bill_num)
    global bill_recommender_2
    bill_recommender_2 = str(int(vendor_billing_list_page.find_approver_id(bill_num)))
    print(f"Bill Recommender 2: {bill_recommender_2}")
    logger.step(f"Bill Recommender 2: {bill_recommender_2}")
    vendor_billing_list_page.get_full_page_screenshot('full_page_screenshot_97')

    m_page = MainNavigationBar(page)
    m_page.exit()
    m_page.logout()
    m_page.get_full_page_screenshot('full_page_screenshot_98')


@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 20: Vendor bill approval by second recommender in the procurement system.
    ...
    Steps:
        ...
        8. Exit and log out from the procurement system, capturing a final screenshot. """)
def test_20_second_recommender_approves_marketplace_bill(page, new_tab, logger):
    """
    Test Case 20: Vendor bill approval by second recommender in the procurement system.
    ...
    Steps:
        ...
        8. Exit and log out from the procurement system, capturing a final screenshot.
    """
    print("Test 20: Vendor bill recommender2 approval...")
    # proc_login_page = ProcurementLoginPage(page)
    # proc_login_page.perform_login(
    #     given_url=proj_url,
    #     user_name=bill_recommender_2,
    #     pass_word=bill_recommender_2_password,
    #     timeout=60000
    # )
    reset_page = ResetHubPage(page)
    ...
    link = reset_page.generate_reset_link(
        env=proj_env,
        username=bill_recommender_2,
    )

    print("Generated Link:" + link)
    reset_page.open_generated_link(link)
    assert isinstance(link, str) and link.startswith("http")
    logger.step(f" 📥 Logging in as bill recommender: {bill_recommender_2}")

    proc_dashboard_page = DashboardPage(page)
    proc_dashboard_page.goto_procurement()
    proc_dashboard_page.get_full_page_screenshot('full_page_screenshot_99')

    proc_home_page = ProcurementHomePage(page)
    proc_home_page.bill_payable.click()
    proc_home_page.goto_vendor_billing_list()
    proc_home_page.get_full_page_screenshot('full_page_screenshot_100')
    ...
    bill_details_information_page = BillDetailsInformation(new_page)
    bill_details_information_page.approve_bill()
    bill_details_information_page.get_full_page_screenshot('full_page_screenshot_101')
    new_page.close()
    ...
    vendor_billing_list_page.get_full_page_screenshot('full_page_screenshot_102')
    global bill_recommender_3
    bill_recommender_3 = str(int(vendor_billing_list_page.find_approver_id(bill_num)))
    print(f"Bill Approver : {bill_recommender_3}")
    logger.step(f"Bill Approver : {bill_recommender_3}")
    vendor_billing_list_page.get_full_page_screenshot('full_page_screenshot_103')
...
@pytest.mark.reporting(
    functional_specification="test_1",
    test_description="""Test Case 21: Vendor bill final approval by the approver in the procurement system.
...
def test_21_vendor_bill_approver_approval_process_in_procurement(page, new_tab):
    """
    Test Case 21: Vendor bill final approval by the approver in the procurement system.
    ...
    Steps:
        ...
        10. Marketplace item received bill for challan number 'challan_num_for_receiver' sent to the HO FIN from the procurement system.
    """
    print("Test 21: Vendor bill approver approval...")
    # proc_login_page = ProcurementLoginPage(page)
    # proc_login_page.perform_login(
    #     given_url=proj_url,
    #     user_name=bill_recommender_3,
    #     pass_word=bill_recommender_3_password,
    #     timeout=60000
    # )
    reset_page = ResetHubPage(page)
    ...
    link = reset_page.generate_reset_link(
        env=proj_env,
        username=bill_recommender_3,
    )

    print("Generated Link:" + link)
    reset_page.open_generated_link(link)
    assert isinstance(link, str) and link.startswith("http")
    logger.step(f" 📥 Logging in as bill approver: {bill_recommender_3}")

    proc_dashboard_page = DashboardPage(page)
    proc_dashboard_page.goto_procurement()
    proc_dashboard_page.get_full_page_screenshot('full_page_screenshot_105')

    proc_home_page = ProcurementHomePage(page)
    proc_home_page.bill_payable.click()
    proc_home_page.goto_vendor_billing_list()
    proc_home_page.get_full_page_screenshot('full_page_screenshot_106')

    vendor_billing_list_page = VendorBillingList(page)
    vendor_billing_list_page.search_bill(bill_num)
    vendor_billing_list_page.get_full_page_screenshot('full_page_screenshot_107')
    new_page = new_tab(lambda p: vendor_billing_list_page.click_on_bill_num(bill_num))

    bill_details_information_page = BillDetailsInformation(new_page)
    bill_details_information_page.wait_for_timeout(5000)
    bill_details_information_page.get_full_page_screenshot('full_page_screenshot_108')
    bill_details_information_page.approve_bill()
    bill_details_information_page.get_full_page_screenshot('full_page_screenshot_109')
    new_page.close()

    vendor_billing_list_page = VendorBillingList(page)
    page.reload()
    vendor_billing_list_page.wait_for_timeout(5000)
    vendor_billing_list_page.get_full_page_screenshot('full_page_screenshot_110')
    vendor_billing_list_page.search_bill(bill_num)
    vendor_billing_list_page.wait_for_timeout(5000)
    bill_status = vendor_billing_list_page.find_bill_status(bill_num)
    print("Bill STATUS:", bill_status)
    logger.step(f"Bill status: {bill_status}")
    vendor_billing_list_page.get_full_page_screenshot('full_page_screenshot_111')
